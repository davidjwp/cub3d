...
/*   Updated: 2024/02/05 19:59:20 by djacobs          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void bresCast(t_mlx *data) {
	...
	float rayAngle = pa - (FOV / 2); //define the ray angle
	float odX;
	float odY;
	...
	for (float i = 0; i < NUM_RAYS; i++) 
	{
		if (i) 
		{
			rdx = odX - (px + LINE * cos(degToRad(rayAngle)));
			rdy = odY - (py + LINE * sin(degToRad(rayAngle)));
		}
		else
		{
			rdx = px + LINE * cos(degToRad(rayAngle));
